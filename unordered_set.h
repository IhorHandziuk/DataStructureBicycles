...
public:
	...
	public:
		...
		typedef typename myVector<T>::const_iterator vec_const_it;

		const_iterator();

		const_iterator(const myVector<myVector<T>>&, const vec_const_it&, const size_t, const size_t);
	...
	private:
		const myVector<myVector<T>>& elemHashesForIters;
		vec_const_it it;
		size_t index;
	...
	public:
		...
		typedef typename myVector<T>::iterator vec_it;

		iterator();

		iterator(myVector<myVector<T>>& hashes, const vec_it & _it, const size_t _index, const size_t last_index);
	...
	private:
		myVector<myVector<T>>& elemHashesForIters;
		vec_it it;
		size_t index;
		const size_t lastIndex; // index of chunk with global end()
	};
...
private:
	myVector<myVector<T>> elemHashes;
	...
	myVector<myVector<T>> createRemappedTable() const;
};
...
inline bool unordered_set<T>::find(const T & k) const {
	size_t hashIndex = fastMod(hasher(k), currPowerOf2);
	const myVector<T> &bucket = elemHashes[hashIndex];
...
template<typename T>
inline myVector<myVector<T>> unordered_set<T>::createRemappedTable() const {
	myVector<myVector<T>> newElemHashes;
...
template<typename T>
inline unordered_set<T>::const_iterator::const_iterator(const myVector<myVector<T>>& hashes, const vec_const_it & _it, const size_t _index, const size_t last_index)
...
template<typename T>
inline unordered_set<T>::iterator::iterator(myVector<myVector<T>>& hashes, const vec_it & _it, const size_t _index, const size_t last_index)
	: elemHashesForIters(hashes), it(_it), index(_index), lastIndex(last_index) {}
