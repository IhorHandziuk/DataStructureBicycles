...
void lock_free_queue::enqueue(int value) 
{
	...
	while (true) {
		...
		if (tailCopy == Tail) {
			if (next == nullptr) {
				if (atomic_compare_exchange_weak_explicit(&tailCopy->next, &next, addNode, memory_order_release, memory_order_relaxed))
					break;
			}
			else {
				atomic_compare_exchange_weak_explicit(&Tail, &tailCopy, next, memory_order_release, memory_order_relaxed);
			}
		}
	}
	...
	while (true)
	{
		...
		if (headCopy == Head) {
			if (headCopy == tailCopy) {
				...
				if (atomic_compare_exchange_weak_explicit(&Head, &headCopy, next, memory_order_release, memory_order_relaxed))
					break;
			}
		}
	}
